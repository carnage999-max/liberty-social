...
export default function PagePostDetailScreen() {
  ...
  const isPagePost = (post as any).author_type === 'page' || ((post as any).page !== null && (post as any).page !== undefined);
  const displayName = isPagePost && (post as any).page
    ? (post as any).page.name
    : buildDisplayName(post.author);
  
  // For page posts, use page profile image; for user posts, use author avatar
  const postAvatarSource = isPagePost && (post as any).page?.profile_image_url
    ? { uri: resolveRemoteUrl((post as any).page.profile_image_url) }
    : post.authorAvatar;
