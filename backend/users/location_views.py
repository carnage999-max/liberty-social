...
def update_user_location(request):
    ...
    if latitude is not None and longitude is not None:
        ...
        logger.info(
            f"User {user.id} updating location from GPS: ({latitude}, {longitude})"
        )
        location_data = {
            "latitude": latitude,
            "longitude": longitude,
    ...
    if not location_source:
        ...
        try:
            ...
            if ip_location:
                ...
                logger.info(
                    f"User {user.id} location detected from IP {client_ip}: {ip_location}"
                )
            else:
                logger.warning(
                    f"Could not determine location for user {user.id} from IP {client_ip}"
                )
                return Response(
                    {
                        "success": False,
