...
class PostFilterForm(forms.Form):
    ...
    show_friend_posts = django_filters.BooleanFilter.field_class(
        required=False, label="Show friend posts"
    )
    show_page_posts = django_filters.BooleanFilter.field_class(
        required=False, label="Show page posts"
    )
    preferred_categories = forms.CharField(required=False, label="Preferred categories")
...
class PostFilterSet(django_filters.FilterSet):
    ...
    show_friend_posts = django_filters.BooleanFilter(
        method="filter_content_type", label="Show friend posts"
    )

    show_page_posts = django_filters.BooleanFilter(
        method="filter_content_type", label="Show page posts"
    )

    preferred_categories = django_filters.CharFilter(
        method="filter_by_category", label="Preferred categories"
    )
    ...
    def filter_by_category(self, queryset, name, value):
        ...
        if not value:
            print(
                f"[FILTER] filter_by_category: no category provided, returning all posts"
            )
            return queryset
        ...
        result = queryset.filter(Q(page__category=value) | Q(page__isnull=True))
