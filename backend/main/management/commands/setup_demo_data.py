...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('🚀 Setting up demo data for screenshots...'))
    ...
    def create_friend_connections(self, users):
        ...
        self.stdout.write(f'  ✓ Created {len(users) * (len(users) - 1) // 2} friendships')
    ...
    def create_sample_messages(self, users):
        ...
        conversations_data = [
            {
                ...
                'messages': [
                    {'sender': users[0], 'content': 'Hey Michael! I saw your post about web development. Could you help me with my website?', 'hours_ago': 3},
                    {'sender': users[1], 'content': 'Hi Sarah! Of course! I\'d be happy to help. What do you need?', 'hours_ago': 2},
                    {'sender': users[0], 'content': 'I need to optimize my site for mobile users. Any recommendations?', 'hours_ago': 2},
                    {'sender': users[1], 'content': 'Definitely! Let\'s start with responsive design. I can send you some resources.', 'hours_ago': 1},
                ]
            },
            {
                'participants': [users[2], users[0]],  # Emma & Sarah
                'messages': [
                    {'sender': users[0], 'content': 'Emma! Your strawberry shortcake looks amazing! 🍰', 'hours_ago': 6},
                    {'sender': users[2], 'content': 'Thank you Sarah! Come by this weekend, I\'ll save you one! 😊', 'hours_ago': 5},
                    {'sender': users[0], 'content': 'Perfect! I\'ll be there Saturday morning!', 'hours_ago': 5},
                ]
            },
            {
                'participants': [users[3], users[4]],  # James & Olivia
                'messages': [
                    {'sender': users[4], 'content': 'James! That 20-mile run is incredible! 💪', 'hours_ago': 10},
                    {'sender': users[3], 'content': 'Thanks Olivia! Training is intense but worth it. How\'s Bali?', 'hours_ago': 9},
                    {'sender': users[4], 'content': 'Amazing! The beaches here are breathtaking. You should visit!', 'hours_ago': 9},
                ]
            },
        ]
        ...
        self.stdout.write(f'  ✓ Created {created_conversations} conversations with {created_messages} messages')
        ...
        for page_data in pages_data:
            ...
            if created:
                ...
                for follower in users[:3]:
                    PageFollower.objects.get_or_create(
                        page=page,
                        user=follower
                    )
    ...
    def create_marketplace_listings(self, users):
        ...
        categories = {}
        category_names = ['Electronics', 'Sports & Outdoors', 'Furniture', 'Home & Garden']
    ...
    def print_demo_accounts(self):
        ...
        self.stdout.write('\n' + '='*60)
        self.stdout.write('💡 TIP: Login with any of these accounts to take screenshots!')
        self.stdout.write('='*60 + '\n')
