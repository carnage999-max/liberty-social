...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        message_count = Message.objects.filter(conversation_id__in=conversation_ids).count()
        ...
        marketplace_count = MarketplaceListing.objects.filter(seller__in=demo_users).count()
        ...
        if not options['confirm']:
            self.stdout.write(self.style.WARNING('⚠️  WARNING: This will permanently delete all demo data!'))
            self.stdout.write('')
        ...
        deleted_messages = Message.objects.filter(conversation_id__in=conversation_ids).delete()
        self.stdout.write(f'   ✓ Deleted {deleted_messages[0]} messages')

        # 2. Delete conversations
        deleted_conversations = Conversation.objects.filter(id__in=conversation_ids).delete()
        ...
        self.stdout.write(f'   ✓ Deleted {deleted_posts[0]} posts (and related likes/comments)')
        ...
        deleted_marketplace = MarketplaceListing.objects.filter(seller__in=demo_users).delete()
